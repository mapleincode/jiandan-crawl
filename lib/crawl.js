...
var executePage = function(pageUrl, callback) {
    ...
    request.get({
        ...
        var number;
        try {
            number = body.match(/\[[0-9]{4}\]/)[0].slice(1, 5);
    ...
    async.waterfall([
        ...
        function(images, number, callback) {
            if (sameNumber && sameNumber !== number) {
                console.log(`set the retry number ${number} after 5 minutes`);
                (function(time){setTimeout(function(){
                    retryNumber = time; 
                    console.log(`success set retry number ${number} at ${new Date()}`);
                }, 300000);})(sameNumber);
            ...
            if(c && number === c.number && lastTimeNumber === number) {
                ...
                if (/*!sameNumber || */sameNumber < number) {
                    ...
                    console.log(`sameNumber has change ${sameNumber} to ${number}...`);
                    sameNumber = number;
                }
        ...
        function(images, imagesAll, currentNumber, callback) {
            ...
            async.eachLimit(list, 1, function(i, callback) {
                i.save(function(err) {
                    if (err) {
                        console.log(`save image url error ${err.message}`);
                    }
                    callback();
                });
            }, function(err) {
